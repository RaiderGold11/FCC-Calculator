...
function App() {
  ...
  const buttonPress = (symbol: string) => {
    if (symbol === "clear") {
      ...
      // don't add decimals to a decimal
  ...
  const calculate = () => {
    //if last symbol is an operator, no calculation
    if(isOperator(et.charAt(et.length - 1))) return;
    //use the last operator in a row of operators
    const parts = et.split(" ");
    const newParts = [];

    //go through expressions backwards
